...
export const TransferCardModal: React.FC<IProps> = ({
    ...
    const changeHandler = (
      event: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>,
        ...
        if(event.currentTarget.name === 'cardAddress' && event.currentTarget.value !== transferCardForm.cardAddress) {
            setTransferCardForm({
                ...transferCardForm,
    ...
    const disabledTransfer = () => {
        if(transferCardForm.tokenId && ethers.utils.isAddress(transferCardForm.toAddress) && ethers.utils.isAddress(transferCardForm.cardAddress)) {
            return false;
        } else {
            return true;
        }
        ...
        <Modal heading="Transfer Cards" onClose={extendedClose}>
          {!loading && !error ? (
            <StyledTransferCardModalContent>
                ...
                <StyledInputRow key={i}>
                  ...
                  {item.type === 'select' && item.name === "tokenId" && (
                      <StyledSelectInput name={item.name} onChange={changeHandler}>
                          ...
                          {
                            profile.ownedAssets.find((ownedAsset) => ownedAsset.assetAddress === transferCardForm.cardAddress)
              ...
              <StyledSaveButton onClick={tranferCard} disabled={disabledTransfer()}>Transfer Card</StyledSaveButton>
            </StyledTransferCardModalContent>
          ) : (
